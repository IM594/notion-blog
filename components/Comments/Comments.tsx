...
const translations = {
  ...
  zh: {
    comments: '评论',
    commentPlaceholder: '在这里写下你的评论...',
    submit: '提交',
    anonymous: '访客',
    unavailableContent: '内容不可用',
  },
};
...
const Comments = ({ pageId, recordMap }: CommentsProps) => {
  ...
  useEffect(() => {
    ...
    if (systemLanguage === 'zh') {
      setLocale(zhCN);
      setCurrentTranslations(translations.zh);
    } else {
      setLocale(enUS);
  ...
  const comments = (data?.results || []).map(item => {
    const user = recordMap.notion_user[item.created_by.id]?.value || {
      id: 'guest',
      name: currentTranslations.anonymous,
    ...
    return {
      id: item.id,
      user: user,
      text: item?.rich_text?.[0]?.plain_text || currentTranslations.unavailableContent,
  ...
  return (
    <div className="notion-comments">
      <h2 className="notion-h notion-h1">{currentTranslations.comments}</h2>
      ...
      <form className={cs('item', loading && 'loading')} onSubmit={formik.handleSubmit}>
        <div className="right">
          <div className="content">
            ...
            <textarea
              name="content"
              placeholder={currentTranslations.commentPlaceholder}
              rows={6}
